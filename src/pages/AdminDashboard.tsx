...
export default function AdminDashboard() {
  ...
  const { data: doctorCount } = trpc.doctors.count.useQuery(undefined, { enabled: activeTab === "overview" });
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-vivavive-light">
        <div className="mx-auto max-w-[1440px] px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-sans text-base font-semibold text-vivavive-dark">VivaVive Admin</h1>
              <p className="font-sans text-xs text-vivavive-muted">Dr. Ishika Patidar&apos;s Clinic</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <Shield className="h-4 w-4 text-vivavive-red" />
              <span className="font-sans text-sm text-vivavive-dark">{user.name || user.email || "Admin"}</span>
            </div>
            <button
              onClick={logout}
          ...
          <main className="flex-1 min-w-0">
            ...
            {activeTab === "overview" && (
              <div className="space-y-6">
                <h2 className="font-serif text-3xl text-vivavive-dark tracking-tight">Dashboard Overview</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard icon={Users} label="Total Patients" value={patientCount ?? 0} color="bg-vivavive-red" />
                  <StatCard icon={Stethoscope} label="Doctors" value={doctorCount ?? 0} color="bg-vivavive-teal" />
                  <StatCard icon={FileText} label="Medical Records" value={recordCount ?? 0} color="bg-vivavive-gray" />
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-sans text-sm text-vivavive-muted">Manage all {tab.label.toLowerCase()}</p>
                        </div>
                      </div>
            ...
            {activeTab === "patients" && (
              <div>
                <h2 className="font-serif text-2xl text-vivavive-dark mb-4">Patients</h2>
                <DataTable
            ...
            {activeTab === "doctors" && (
              <div>
                <h2 className="font-serif text-2xl text-vivavive-dark mb-4">Doctors</h2>
                <DataTable
                  headers={["Name", "Email", "Specialization", "Experience", "Status"]}
                  rows={doctors?.map((d) => ({
                    id: d.id,
                    cells: [
                      d.name,
                      ...
                      <span key={d.id} className={cn("rounded-full px-2.5 py-1 text-xs font-medium", d.isActive === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600")}>
                        {d.isActive}
                      </span>,
                    ],
                  })) ?? []}
